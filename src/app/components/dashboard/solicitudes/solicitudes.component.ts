...
export class SolicitudesComponent {
  ...
  seleccionarSolicitues(){
    for(let index = 0; index < this.listaSolicitudes.length; index++){
        if(this.listaSolicitudes[index].tipo=="Nueva"){
          this.listaSolicitudesEspecificas.push(this.listaSolicitudes[index]);
        }
    }
    this.dataSource = new MatTableDataSource(this.listaSolicitudesEspecificas);
  }
